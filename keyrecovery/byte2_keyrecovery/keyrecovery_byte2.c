...
u16* getk(u8 key_num,u16 **Ps,u16 **Cs,u32 LEN)
{
    /*
    ...
    */
    ...
    if(key_num == 1)
    {
        ...
        */
        ...
        for(int p = 0;p<10;p++)
        {
            ...
            {
                if(byte2(Ps[p][0],k_try,2) == Cs[p][0] && byte2(Ps[p][1],k_try,2) == Cs[p][1])
                {
                    keys[k_try] = 1;
                }
                else
                {
                    if(keys[k_try] == 1) keys[k_try] = 0;
                }
            }
            ctr = 0;
    ...
    {
        alpha = 49344;
        alpha_table = getAlphaTable("./2alpha49344");   
    }
    else if(key_num == 3) // Recover k_4 and k_5
    {
        alpha = 49152;
        alpha_table = getAlphaTable("./4alpha49152");     
    }
    else if(key_num == 4) // Recover k_6 and k_7
    {
        alpha = 32832;
        alpha_table = getAlphaTable("./6alpha32832");
    }
...
u64 keyrecovery(u8 key_nr,u16 ***Ps,u16 ***Cs,u32 LEN,u64 currently_guessed_key)
{
    /*
        Recursively runs getk to recover the correct key k_B2.
        It starts by creating the scoreboard of (k_B2,6 , k_B2,7), uses the top key of that scoreboard to 
        ...
        key_nr: the rounds which we are attacking (valid values: 1-4)
            1: Recovers (k_B2,0 , k_B2,1)
            2: Recovers (k_B2,2 , k_B2,3)
            3: Recovers (k_B2,4 , k_B2,5)
            4: Recovers (k_B2,6 , k_B2,7)
        Ps: Array of plaintext-pairs with input difference alpha.
        Cs: Array of encrypted Ps
        LEN: Number of plaintext-pairs
    ...
    */
    ...
    while((key_cand = key_candidates[nr++])) // Walk through scoreboard and try to continue attack
    {
        for(int r = 4;r>0;r--)
            for (int i = 0;i<LEN;i++)
            {
                Cs[r][i][0] = byte2_dec(Cs[r][i][0],key_cand,2);
                Cs[r][i][1] = byte2_dec(Cs[r][i][1],key_cand,2);
            }
        // ********************* RECURSION ********************* //
        u64 new_key =  keyrecovery(key_nr-1,Ps,Cs,LEN,currently_guessed_key | ((u64) key_cand <<16*(4-key_nr)));
        // ***************************************************** //
        if(new_key != log(0)) return new_key;
        for(int r = 4;r>0;r--)
            for (int i = 0;i<LEN;i++)
            {
                Cs[r][i][0] = byte2(Cs[r][i][0],key_cand,2);
                Cs[r][i][1] = byte2(Cs[r][i][1],key_cand,2);
            }
    }
