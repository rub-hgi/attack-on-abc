...
state64 rotateXOR(state64 input,int round)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        out.data[i] = ((u_int8_t) (new_left/(1<<(8*(3-i)))));// % (1<<8);
        out.data[i+4] = ((u_int8_t) (new_right/(1<<(8*(3-i)))));// % (1<<8);
    }
...
{
    state128 state = {0,0,p>>8,0,0,0,0,0,0,0,p & 0xff,0,0,0,0,0};
    for (int i = (rounds-1); i >= 0;i--)
    {
        state64 _k = {0,0,(k>>(8*i)) & 0xff,0,0,0,0,0}; // state64 ist hier ein struct uint8[8], jeder Eintrag ist mod 256 
        state = runde(state,_k,false);
    }
    u16 output = (state.data[2] << 8) + state.data[10];
    return output;
}
...
u128 readK(char* arg) 
{
    ...
    char* end;
    u128 p = 0;
    size_t len_second_chunk = 0;
    if(len > 16)
    {
        len_second_chunk = strlen(&arg[16]);
        p = (u128) strtoull(&arg[16],&end,16);
        if(strlen(end) != 0) die("K was read wrong");
    }
