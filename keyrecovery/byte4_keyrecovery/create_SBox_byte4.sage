...
def byte4(p,k,rounds=4):
    return byte4_c(p,k,rounds)    

def byte4_c(p,k,rounds=4):
    _stdout = str(subprocess.run("./byte4 "+hex(p)[2:]+" "+hex(k)[2:]+" "+hex(rounds)[2:], shell=True ,capture_output=True))
