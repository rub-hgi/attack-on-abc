/*
    This file implements the B4 (byte 4) cipher. Most of the functions are implemented in byte4_linker,
    this file just makes the execution easier.

    Compile with: make / make byte4
    Run with: ./byte4 <p> <k> <r> [-inv]
...
int main(int argc, char* argv[])
{
    ...
    u32 c;
    if (argc > 4 && !strcmp(argv[4],"inv")) c = byte4_dec(p,k,r);
    else c = byte4(p,k,r);

    printf("%x\n",c);

    if(byte4_dec(byte4(p,k,r),k,r) == p) return EXIT_SUCCESS;
